...
class ImportFigmaProductImagesCommand extends Command
{
    ...
    private function attachImageToProduct(
        ...
        $contentHash = substr(sha1($normalizedBinary), 0, 12);
        $relativePath = $this->buildTargetRelativePath($product, $file, $sourcePath, $contentHash);
        $alreadyExists = $product->productImages
            ->contains(fn (ProductImage $image): bool => $image->path === $relativePath);
    ...
    private function buildTargetRelativePath(
        Product $product,
        SplFileInfo $file,
        string $sourcePath,
    ...
    ): string
    {
        ...
        $variantHash = $contentHash !== null && $contentHash !== ''
            ? $contentHash
            : substr(md5($this->relativePath($file, $sourcePath)), 0, 12);
